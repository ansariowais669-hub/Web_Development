...
const Manager = () => {
    ...
    const getPasswords = async () => {
        ...
        const formatted = passwords.map(item => ({
            ...item,
            _id: item._id.toString()
        }))

        setPasswordArray(formatted)
    }
    ...
    const savePassword = async () => {
        ...
        setPasswordArray([
            ...passwordArray,
            { ...form, _id: data.result.insertedId.toString() }
        ])
                                    ...
                                    <tbody className='bg-green-100'>
                                        {passwordArray.map((item, index) => {
                                            return <tr key={item._id}>
                                                <td className='text-center w-32 py-2 border border-white'><a href={item.site} target='_blank' rel="noreferrer">{item.site}</a></td>
